...
class RegClassTest(fixtures.TestBase):
    ...
    def _scalar(expression):
        return select([expression], bind=testing.db).scalar()

    def test_cast_name(self):
        assert self._scalar(cast('pg_class', postgresql.REGCLASS)) == 'pg_class'

    def test_cast_path(self):
        assert self._scalar(cast('pg_catalog.pg_class', postgresql.REGCLASS)) == 'pg_class'

    def test_cast_oid(self):
        regclass = cast('pg_class', postgresql.REGCLASS)
        oid = self._scalar(cast(regclass, postgresql.OID))
        assert isinstance(oid, int)
        assert self._scalar(cast(oid, postgresql.REGCLASS)) == 'pg_class' 
    ...
    def test_cast_whereclause(self):
        ...
        oid, _ = pga.select()\
            .where(pga.c.attrelid == cast('pg_class', postgresql.REGCLASS))\
            .execute().first()
        assert isinstance(oid, int)
        assert self._scalar(cast(oid, postgresql.REGCLASS)) == 'pg_class'
